...
class OllamaService {
    ...
    async analyzeResumeMatch(jdText, resumeText, settings) {
        try {
     ...
     * 创建分析提示词
     */
    createAnalysisPrompt(jdText, resumeText, settings) {
...
请严格按照上述JSON格式输出分析结果，不要添加其他文字说明。`;
    ...
    normalizeConclusion(conclusion) {
        ...
        if (normalized.includes('STRONG'))
            return 'STRONG_HIRE';
        if (normalized.includes('推荐') || normalized.includes('HIRE'))
            return 'HIRE';
        if (normalized.includes('倾向') && normalized.includes('不'))
            return 'LEAN_NO';
