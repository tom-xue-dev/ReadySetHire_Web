...
describe('Database Connection', () => {
    test('should connect to test database', async () => {
        const result = await setup_1.testPrisma.$queryRaw `SELECT 1 as test`;
        expect(result).toBeDefined();
    });
    test('should have all required tables', async () => {
        const tables = await setup_1.testPrisma.$queryRaw `
      SELECT table_name 
    ...
    test('should have proper foreign key constraints', async () => {
        const constraints = await setup_1.testPrisma.$queryRaw `
      SELECT 
        tc.constraint_name,
        tc.table_name,
        kcu.column_name,
...
//# sourceMappingURL=database-connection.test.js.map