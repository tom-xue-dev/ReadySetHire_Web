        ...
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-6 mb-6">
          ...
          <Card className="max-w-none p-6">
            ...
            <div className="flex gap-2 mb-4">
              ...
              <button
                type="button"
                onClick={() => setResumeInput({ ...resumeInput, method: 'select' })}
                className={`px-3 py-1.5 text-sm rounded-lg ${
                  resumeInput.method === 'select'
                    ? 'bg-green-100 text-green-700 font-medium'
                    : 'bg-gray-100 text-gray-600 hover:bg-gray-200'
                }`}
              >
                {t('rateResume.resume.select')}
              </button>
            </div>
            ...
            {resumeInput.method === 'select' && (
              <div className="mb-4">
                <label className="block text-sm font-medium text-gray-700 mb-2">{t('rateResume.resume.select')}</label>
                <select
                  value={resumeInput.selectedCandidateId || ''}
                  onChange={(e) => {
                    setResumeInput({ ...resumeInput, selectedCandidateId: e.target.value });
                    // TODO: Load resume from selected candidate
                  }}
                  className="w-full px-3 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-green-500 focus:border-transparent"
                >
                  <option value="">{t('rateResume.resume.selectPlaceholder')}</option>
                  <option value="1">张三 - 高级前端工程师</option>
                  <option value="2">李四 - 全栈开发工程师</option>
                </select>
              </div>
            )}
